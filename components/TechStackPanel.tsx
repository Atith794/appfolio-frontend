...
type TechStackCategory = Extract<
  TechCategory,
...
const LIMITS: Record<TechStackCategory, number> = {
  frontend: 6,
  backend: 6,
  database: 3,
  infra: 6,
...
type TechStackState = Record<TechStackCategory, string[]>;
...
export function TechStackPanel({ appId }: { appId: string }) {
  ...
  const [active, setActive] = useState<TechStackCategory>("frontend");
  ...
  async function load(isReset: boolean) {
    ...
    try {
      ...
      const ts = (app?.techStack || {}) as Partial<TechStackState>;

      const next: TechStackState = {
        frontend: uniq(normalizeIds(ts.frontend || [])).slice(
          0,
          LIMITS.frontend,
        ),
        backend: uniq(normalizeIds(ts.backend || [])).slice(0, LIMITS.backend),
        database: uniq(normalizeIds(ts.database || [])).slice(
          0,
          LIMITS.database,
        ),
  ...
  function isTechStackCategory(
    category: TechCategory,
  ): category is TechStackCategory {
    return TECH_STACK_CATEGORIES.includes(category as TechStackCategory);
  }
  ...
  function addTech(item: TechItem) {
    ...
    if (!isTechStackCategory(category)) return;

    setStack((prev) => {
      const current = prev[category];

      if (current.includes(item.id)) return prev;
      if (current.length >= LIMITS[category]) return prev;
  ...
  function removeTech(category: TechStackCategory, id: string) {
    setStack((prev) => ({
      ...prev,
      ...
      <div className="border-2 border-dashed border-slate-200 rounded-2xl p-4 hover:border-primary/50 hover:bg-primary/5 transition-all">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Pick the technologies used to build this app. These icons will
              appear on your public AppShelves portfolio.
            </p>
          </div>
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between gap-2">
            ...
            <p
              className={`text-xs font-medium ${limitReached ? "text-red-600" : "text-slate-500"}`}
            >
          ...
          {stack[active].length === 0 ? (
            <p className="mt-2 text-xs text-slate-500">
              No tech selected yet for this category.
            </p>
          ) : (
                  ...
                  >
                    {item?.iconClass ? (
                      <div className="h-6 w-6 grid place-items-center rounded-md bg-white/80">
                        <i
                          className={`${item.iconClass} colored text-lg dark:opacity-90`}
                        />
                      </div>
                    ) : (
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                >
                  {item.iconClass ? (
                    <div className="h-6 w-6 grid place-items-center rounded-md bg-white/90">
                      <i
                        className={`${item.iconClass} colored text-xl dark:opacity-90`}
                      />
                    </div>
                  ) : (
          ...
          <p className="mt-3 flex items-center gap-2 text-xs font-medium text-primary/80">
            <Info size={14} />
            <span>
              Tip: Keep it tight — recruiters scan this in 2 seconds. Frontend{" "}
              {LIMITS.frontend}, Backend {LIMITS.backend}, Database{" "}
              {LIMITS.database}, Infra {LIMITS.infra}.
            </span>
          </p>
        </div>
